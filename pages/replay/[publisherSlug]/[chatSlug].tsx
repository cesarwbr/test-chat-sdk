...
function generateNewMessages(allMessages, newStrings) {
  ...
  allMessages.forEach((message) => {
    newMessages.push({
      ...
      sender: {
        ...message.sender,
        photoURL: "https://i.pravatar.cc/32?u=" + message.sender.uid,
      },
    });

    if (newStrings[currentIndex + 1]) {
      currentIndex++;
    } else {
      currentIndex = 0;
    }
...
const newHistory = generateNewMessages(history, newStrings);
console.clear();
...
export default function App() {
  ...
  useEffect(() => {
    async function callNextMessage(message: any, index: number) {
      ...
      const nextMessage = newHistory[index + 1];
      if (nextMessage) {
    ...
    if (currentMessageIndex !== null && newHistory[currentMessageIndex]) {
      const message = newHistory[currentMessageIndex];
